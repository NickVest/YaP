    ...
    private void button3_Click(object sender, EventArgs e)
    {
      ...
      for (int i = 0; i < bmp.Width; i++)
      {
        for (int j = 0; j < bmp.Height; j++)
        {
          ...
          Color p = Color.FromArgb(255, Gray*0, Gray*0, Gray); //переводим int в значение цвета. 255 - показывает степень прозрачности. остальные значения одинаковы для трех каналов R,G,B
          if (!(i > (400 - sc) && i < (400 + sc) && j > 100 && j < 300))
          {
            bmp.SetPixel(i, j, p); //записываме полученный цвет в текущую точку

          }
          if (i== (400 + sc) && j == (100 + sc))
          {
            sc++;
          }
